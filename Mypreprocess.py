...
parser.add_argument('-train_pool', required=True,
                    help="Path to the training pool data")
parser.add_argument('-valid_src', required=True,
                    help="Path to the validation source data")
parser.add_argument('-valid_tgt', required=True,
                    help="Path to the validation target data")
parser.add_argument('-valid_pool', required=True,
                    help="Path to the validation pool data")
...
def main():
    ...
    fields = onmt.IO.MyONMTDataset.get_fields(nFeatures)
    print("Building Training...")
    train = onmt.IO.MyONMTDataset(opt.train_src, opt.train_tgt, opt.train_pool, fields, opt)
    print("Building Vocab...")
    onmt.IO.MyONMTDataset.build_vocab(train, opt)

    print("Building Valid...")
    valid = onmt.IO.MyONMTDataset(opt.valid_src, opt.valid_tgt, opt.valid_pool, fields, opt)
    print("Saving train/valid/fields")

    # Can't save fields, so remove/reconstruct at training time.
    torch.save(onmt.IO.MyONMTDataset.save_vocab(fields),
