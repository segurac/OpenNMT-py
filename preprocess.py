...
def build_save_text_dataset_in_shards(src_corpus, tgt_corpus, fields,
    ...
    while not src_iter.hit_end():
        index += 1
        #init_token is None, because we will add the token at the tgt text directly to be able to have multiple tgt languages
        dataset = onmt.io.TextDataset(
                fields, src_iter, tgt_iter,
                src_iter.num_feats, tgt_iter.num_feats,
                src_seq_length=opt.src_seq_length,
                tgt_seq_length=opt.tgt_seq_length,
                dynamic_dict=opt.dynamic_dict, init_token=None)
