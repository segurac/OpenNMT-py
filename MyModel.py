...
class MyAttetion_RNN_CNN(nn.Module):
    def __init__(self, vocab_size, embedding_dim, hidden_size, n_filters, window_size,
                 n_layers, fc_size, dropout, batch_size, bidirectional, use_cuda):
        """
        Args:
            input (LongTensor): len x batch x nfeat.
            lengths (LongTensor): batch
        ...
        outputs (FloatTensor):  len x batch x rnn_size -  Memory bank
        """
        super(MyAttetion_RNN_CNN, self).__init__()
        self.use_cuda = use_cuda
        self.n_layers = n_layers
        self.batch_size = batch_size
        self.hidden_size = hidden_size
    ...
    def forward(self, question, c_answers, n_answers, lengths, batch_size, isvalid):
        ...
        oq, _ = torch.max(q_outputs, 0)  # MaxPooling
        ...
        a_outputs = torch.transpose(a_outputs, 0, 1)
        aux1 = self.fc_am(a_outputs)
        aux2 = self.fc_qm(oq)
        aux2 = torch.unsqueeze(aux2, 1)
        maq = aux1 + aux2
        maq = F.tanh(maq)

        saq = torch.exp(self.fc_ms(maq))

        a_outputs = a_outputs*saq   # Update hidden statees
...
class MyAttentionRNN(nn.Module):
    def __init__(self, vocab_size, embedding_dim, hidden_size, n_layers,
                 fc_size, dropout, batch_size, bidirectional, use_cuda):
        """
        Args:
            input (LongTensor): len x batch x nfeat.
            lengths (LongTensor): batch
        ...
        outputs (FloatTensor):  len x batch x rnn_size -  Memory bank
        """
        super(MyAttentionRNN, self).__init__()
        self.use_cuda = use_cuda
        self.n_layers = n_layers
        self.batch_size = batch_size
        self.hidden_size = hidden_size
